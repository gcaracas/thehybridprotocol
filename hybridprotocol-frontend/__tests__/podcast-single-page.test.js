...
import PodcastSinglePage from '@/app/podcasts-single/[slug]/page';
...
describe('PodcastSinglePage Component', () => {
  ...
  describe('Loading State', () => {
    it('should display loading state initially', async () => {
      ...
      await act(async () => {
        render(<PodcastSinglePage params={mockParams} />);
      });
  ...
  describe('Successful Podcast Display', () => {
    beforeEach(async () => {
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
    ...
    it('should display navigation link', async () => {
      await waitFor(() => {
        expect(screen.getByText('Back to Podcasts')).toBeInTheDocument();
      });
    });

    it('should have correct navigation URL', async () => {
      await waitFor(() => {
        const backLink = screen.getByText('Back to Podcasts').closest('a');
        expect(backLink).toHaveAttribute('href', '/podcasts');
      });
    });
  });

  describe('Script Section', () => {
    it('should display script section when script is available', async () => {
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('This episode has a detailed script with timestamps, key points, and full transcript.')).toBeInTheDocument();
        expect(screen.getByText('View Full Script')).toBeInTheDocument();
      });
    });

    it('should have correct script link URL', async () => {
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      ...
      await waitFor(() => {
        const scriptLink = screen.getByText('View Full Script').closest('a');
        expect(scriptLink).toHaveAttribute('href', '/podcasts-single/test-podcast/script');
      });
    });

    it('should not display script section when script is not available', async () => {
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      ...
      await waitFor(() => {
        expect(screen.queryByText('Episode Script')).not.toBeInTheDocument();
        expect(screen.queryByText('View Full Script')).not.toBeInTheDocument();
      });
    });

    it('should have proper styling for script button', async () => {
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      ...
      await waitFor(() => {
        const scriptButton = screen.getByText('View Full Script').closest('a');
        expect(scriptButton).toHaveClass('btn', 'btn-primary');
      });
    });

    it('should include script icon in the button', async () => {
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      ...
      await waitFor(() => {
        const scriptButton = screen.getByText('View Full Script');
        expect(scriptButton).toBeInTheDocument();
        // The icon should be present (though we can't easily test the icon class in this mock setup)
      });
    });
  });
  ...
  describe('Component Props and Dependencies', () => {
    it('should handle missing params gracefully', async () => {
      ...
      await act(async () => {
        render(<PodcastSinglePage params={{}} />);
      });
    ...
    it('should have all required imports available', async () => {
      // This test ensures that all required imports are available
      // If any import is missing, this test will fail
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      ...
      await waitFor(() => {
        // If we get here without errors, all imports are working
        expect(screen.getByText('Podcast Episode:')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Accessibility', () => {
    it('should have proper heading structure', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Podcast Episode:');
        const h4Elements = screen.getAllByRole('heading', { level: 4 });
        const listenHeading = h4Elements.find(el => el.textContent === 'Listen to this episode');
    ...
    it('should have proper link text for navigation', async () => {
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
  ...
  describe('Rich Text Support', () => {
    it('should render title with rich text formatting', async () => {
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
    ...
    it('should render description with rich text formatting', async () => {
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
      mockedApiService.getPodcastEpisodeBySlug.mockResolvedValue(mockPodcast);
